...
class App {
  ...
  async checkUpdate(){this.updateBusy.set(true);this.updateVersion.set('');this.updateStatus.set('Checking GitHub Releases…');try{const result=await this.bridge.call<{current:string;latest:string|null}>('check-update');this.updateVersion.set(result.latest??'');this.updateStatus.set(result.latest?`Version ${result.latest} is available (installed: ${result.current}).`:`Version ${result.current} is current, or no installer release is published yet.`);}catch(e){this.updateStatus.set((e as Error).message);}finally{this.updateBusy.set(false);}}
  async installUpdate(){this.updateBusy.set(true);this.updateStatus.set('Downloading and verifying installer…');try{await this.bridge.call('install-update');this.updateVersion.set('');this.updateStatus.set('Installer started.');}catch(e){this.updateStatus.set((e as Error).message);this.updateVersion.set('');}finally{this.updateBusy.set(false);}}
